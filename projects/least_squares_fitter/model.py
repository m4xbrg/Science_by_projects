...
def _apply_weights(X: np.ndarray, y: np.ndarray, w: Optional[np.ndarray]) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray]]:
    if w is None:
        return X, y, None
    w = np.asarray(w)
...
def _ridge_augmented_qr(X: np.ndarray, y: np.ndarray, ridge_lambda: float) -> Tuple[np.ndarray, np.ndarray]:
    if ridge_lambda <= 0.0:
        return X, y
    p = X.shape[1]
...
def _solve_beta(X: np.ndarray, y: np.ndarray, solver: str, ridge_lambda: float) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    """
...
def fit_least_squares(X: np.ndarray, y: np.ndarray, cfg: LSConfig) -> LSResult:
    ...
    meta = dict(n=int(n), p=int(p), add_intercept=cfg.add_intercept,
                solver=cfg.solver, ridge_lambda=float(cfg.ridge_lambda))
    return LSResult(beta=beta, y_hat=y_hat, residuals=residuals,
                    sigma2_hat=s2, H_diag=H_diag,
                    std_resid=std_resid, cooks_d=cooks_d, meta=meta)
