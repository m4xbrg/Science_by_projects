...
def to_float(arr): return arr.astype(np.float64, copy=False)
def dynamic_range(arr): return 255.0 if arr.max() <= 255.0 else arr.max()
...
def _local_stats(x, win_size, sigma):
    ...
    mu_sq = mu * mu
    sigma_sq = gaussian_filter(x * x, sigma=sigma, truncate=((win_size - 1) / 2) / sigma) - mu_sq
    return mu, sigma_sq
...
def ssim_mean(x, y, data_range, win_size=11, sigma=1.5, K1=0.01, K2=0.03):
    if x.ndim == 2:
        ...
        vals = [ssim_single(x[..., c], y[..., c], data_range, win_size, sigma, K1, K2) for c in range(x.shape[2])]
