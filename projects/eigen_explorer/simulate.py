...
def run(config_path: str):
    ...
    x0 = np.array(cfg.get('x0', np.random.default_rng(cfg.get('seed', 0)).normal(size=A.shape[0])), dtype=float)
    tol = cfg['power_method']['tol']
    max_iter = cfg['power_method']['max_iter']
    norm = cfg['power_method'].get('normalize', 'l2')

    lam_pm, x_pm, hist = power_method(A, x0, tol=tol, max_iter=max_iter, norm=norm, return_history=True)
    ...
    comp = {
        ...
        "angle_between_deg": float(np.degrees(np.arccos(
            np.clip(np.abs(np.dot(h['x'], v_dom)/ (np.linalg.norm(h['x'])*np.linalg.norm(v_dom))), 0.0, 1.0)
        ))) if hist else None
    }
