...
def _rv_uniform(rng: np.random.Generator, size: int, a: float = 0.0, b: float = 1.0) -> np.ndarray:
    return rng.uniform(a, b, size=size)

def _rv_exponential(rng: np.random.Generator, size: int, lam: float = 1.0) -> np.ndarray:
...
def population_moments(name: str, params: Dict) -> Tuple[float, float]:
    ...
    if name == "pareto":
        ...
        if alpha <= 2:
            raise ValueError("Pareto variance is infinite for alpha<=2; CLT does not apply in classical form.")
        mu = alpha/(alpha-1.0)
