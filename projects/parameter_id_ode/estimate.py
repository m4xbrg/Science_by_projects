...
def nls_estimate(t: np.ndarray, y: np.ndarray, x0: np.ndarray, theta0: np.ndarray) -> Dict:
    """
    ...
    res = least_squares(resid, theta0, method="trf", jac="2-point", xtol=1e-10, ftol=1e-10, gtol=1e-8, max_nfev=200)
    theta_hat = res.x
    r = res.fun
...
def collocation_estimate(t: np.ndarray, y: np.ndarray, smoothing: float, theta0: np.ndarray) -> Dict:
    """
    ...
    res = least_squares(residual, theta0, method="trf", jac="2-point", xtol=1e-12, ftol=1e-12, gtol=1e-10, max_nfev=200)
    theta_hat = res.x
    r = res.fun
    dof = len(y) - len(theta_hat)
    sigma2 = (r @ r) / max(dof, 1)
...
def map_estimate(t: np.ndarray, y: np.ndarray, x0: np.ndarray, theta0: np.ndarray, mu0: np.ndarray, Sigma0: np.ndarray) -> Dict:
    """
    ...
    res = least_squares(resid, theta0, method="trf", jac="2-point", xtol=1e-10, ftol=1e-10, gtol=1e-8, max_nfev=300)
    theta_hat = res.x
    r = res.fun
...
def main(config_path: str, method: str):
    ...
    theta0 = np.array([cfg["estimation"]["initial_guess"]["theta1"],
                       cfg["estimation"]["initial_guess"]["theta2"]], dtype=float)
...
if __name__ == "__main__":
    ...
    p.add_argument("--method", type=str, required=True, choices=["nls","collocation","map"])
    args = p.parse_args()
