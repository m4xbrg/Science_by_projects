...
def slope_field_and_isoclines(cfg, save_path="figs/slope_isoclines.png"):
    ...
    CS = plt.contour(
        T, X, F, levels=cfg.get("isocline_levels", [-2, -1, 0, 1, 2]), linestyles="--"
    )
    plt.clabel(CS, inline=True, fontsize=8)
    dt = sorted(cfg["dt_list"])[-1]
    t_vec, x_vec = integrate(
        f, cfg["t0"], cfg["x0"], cfg["t_final"], dt, method="rk4", params=P
    )
...
def error_vs_stepsize(
    cfg, df_path="results.parquet", save_path="figs/error_vs_stepsize.png"
):
    ...
    for method, sub in df.groupby("method"):
        ...
        plt.loglog(sub["dt"], sub["global_error"], marker="o", label=method)
    ...
    plt.xlabel("Δt (step size)")
    plt.ylabel("Global error at t_final")
    plt.legend(loc="best")
...
if __name__ == "__main__":
    cfg = load_cfg()
    slope_field_and_isoclines(cfg)
    error_vs_stepsize(cfg)
