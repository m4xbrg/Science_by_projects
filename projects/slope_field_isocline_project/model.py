...
_SAFE_NS = {"np": np, "sin": np.sin, "cos": np.cos, "tan": np.tan,
...
def parse_rhs(expr: str) -> Callable[[float, float, Dict[str, float]], float]:
    ...
    def f(t: float, x: float, params: Dict[str, float]) -> float:
        ns = dict(_SAFE_NS); ns["t"] = t; ns["x"] = x; ns.update(params or {})
...
def integrate(f: Callable, t0: float, x0: float, t_final: float, h: float, method: str, params: Dict):
    step = euler_step if method.lower() == "euler" else rk4_step
    import numpy as np
    n = int(np.ceil((t_final - t0)/h))
    t = t0; x = x0
    t_vec = np.empty(n+1); t_vec[0]=t0
    x_vec = np.empty(n+1); x_vec[0]=x0
    for i in range(1, n+1):
        t, x = step(f, t, x, h, params); t_vec[i]=t; x_vec[i]=x
    return t_vec, x_vec
