...
def run(config_path: str = "config.yaml") -> Dict:
    ...
    X_tr, X_tmp, y_tr, y_tmp = train_test_split(
        X, y, train_size=cfg.splits["train"], random_state=seed, stratify=y
    )
    rel = cfg.splits["val"] / (cfg.splits["val"] + cfg.splits["test"])
    X_va, X_te, y_va, y_te = train_test_split(
        X_tmp, y_tmp, train_size=rel, random_state=seed, stratify=y_tmp
    )
    ...
    metrics = {
        "train": evaluate_all(
            y_tr, p_tr, cfg.evaluation["thresholds"], cfg.evaluation["n_bins"]
        ),
        "val": evaluate_all(
            y_va, p_va, cfg.evaluation["thresholds"], cfg.evaluation["n_bins"]
        ),
        "test_raw": evaluate_all(
            y_te, p_te, cfg.evaluation["thresholds"], cfg.evaluation["n_bins"]
        ),
        "test_cal": evaluate_all(
            y_te, p_te_cal, cfg.evaluation["thresholds"], cfg.evaluation["n_bins"]
        ),
    }
    ...
    return {
        "metrics": metrics,
        "paths": {"summary": os.path.join(out_dir, cfg.io["parquet_name"])},
    }
