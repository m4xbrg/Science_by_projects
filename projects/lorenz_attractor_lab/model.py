...
def _plane_eval(x: np.ndarray, plane: str) -> float:
    if plane.startswith("x="):
        return x[0] - float(plane.split("=")[1])
    if plane.startswith("y="):
        return x[1] - float(plane.split("=")[1])
    if plane.startswith("z="):
        return x[2] - float(plane.split("=")[1])
...
def poincare_crossings(ts, xs, plane="x=0", direction="increasing"):
    ...
    for i in range(len(ts) - 1):
        a, b = vals[i], vals[i + 1]
        if direction == "increasing" and not (a < 0 <= b):
            continue
        if direction == "decreasing" and not (a > 0 >= b):
            continue
        if a == b:
            lam = 0.0
        else:
