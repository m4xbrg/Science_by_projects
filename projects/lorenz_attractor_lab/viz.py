...
def plot_poincare(df, outpath, plane="x=0", direction="increasing"):
    ...
    if len(pts)>0:
        if plane.startswith("x="): ax.scatter(pts[:,1], pts[:,2], s=5); ax.set_xlabel("y"); ax.set_ylabel("z")
        elif plane.startswith("y="): ax.scatter(pts[:,0], pts[:,2], s=5); ax.set_xlabel("x"); ax.set_ylabel("z")
        else: ax.scatter(pts[:,0], pts[:,1], s=5); ax.set_xlabel("x"); ax.set_ylabel("y")
    ax.set_title(f"Poincaré Section on {plane}")
    fig.tight_layout(); fig.savefig(outpath, dpi=180); plt.close(fig)

def plot_sensitivity(df, outpath, epsilon=1e-6, sigma=10.0, rho=28.0, beta=8.0/3.0):
    t = df["t"].to_numpy(); dt = t[1]-t[0]
    X = df[["x","y","z"]].to_numpy()
    x0 = X[0].copy(); x0[0] += epsilon
    def rhs(x): return np.array([sigma*(x[1]-x[0]), x[0]*(rho-x[2]) - x[1], x[0]*x[1] - beta*x[2]])
    def rk4_step(x):
        k1=rhs(x); k2=rhs(x+0.5*dt*k1); k3=rhs(x+0.5*dt*k2); k4=rhs(x+dt*k3); return x + (dt/6.0)*(k1+2*k2+2*k3+k4)
    Y = np.zeros_like(X); y=x0.copy()
    for i in range(len(t)):
        Y[i]=y; y=rk4_step(y)
    sep = np.linalg.norm(Y - X, axis=1)
    fig = plt.figure(figsize=(6,4)); ax = fig.add_subplot(111)
    ax.plot(t, np.log(sep)); ax.set_xlabel("t"); ax.set_ylabel("log ||δx(t)||"); ax.set_title("Sensitivity to Initial Conditions")
    fig.tight_layout(); fig.savefig(outpath, dpi=180); plt.close(fig)
