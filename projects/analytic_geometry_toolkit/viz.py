...
def plot_scene(points, lines, circles, viewport=None, out_path="figs/scene.png"):
    ...
    # Viewport
    if viewport is None:
        xs = [x for x, y in points] + [c.x0 - c.r for c in circles] + [c.x0 + c.r for c in circles]
        ys = [y for x, y in points] + [c.y0 - c.r for c in circles] + [c.y0 + c.r for c in circles]
        if xs and ys:
            margin = 0.1 * max((max(xs) - min(xs) + max(ys) - min(ys)), 1.0)
            viewport = (min(xs) - margin, max(xs) + margin, min(ys) - margin, max(ys) + margin)
        else:
            viewport = (-5, 5, -5, 5)
    ...
    for i, L in enumerate(lines):
        ...
        if len(candidates) >= 2:
            ...
            ax.add_line(Line2D([p_start[0], p_end[0]], [p_start[1], p_end[1]], label='Line' if i == 0 else None))
        ax.annotate(f"L{i}", ((xmin+xmax)/2, (ymin+ymax)/2), xytext=(0, 0), textcoords='offset points')
...
def main(config_path="config.yaml"):
    ...
    plot_distance_matrix(points, out_path=fig_dir / cfg["output"]["fig_distance_matrix"])
