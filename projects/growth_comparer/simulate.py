...
def integrate_model(model: str, params: dict, y0: float, t_final: float, dt: float) -> Tuple[np.ndarray, np.ndarray]:
    validate_params(model, params)
    t_eval = np.arange(0.0, t_final + 1e-12, dt)
    fun = lambda t, y: rhs(t, y, params, model)
    sol = solve_ivp(fun, (0.0, t_final), y0=np.array([y0], dtype=float), t_eval=t_eval, rtol=1e-9, atol=1e-12)
    if not sol.success:
        raise RuntimeError(f"solve_ivp failed for {model}: {sol.message}")
    return sol.t, sol.y[0]

def _bisection_root(func: Callable[[float], float], a: float, b: float, tol: float=1e-6, maxit: int=100):
    fa, fb = func(a), func(b)
...
def main():
    ...
    for i, j in pairs:
        ...
        cross[f"{i}__vs__{j}"] = first_nontrivial_crossover(ti, yi, yj, tol=float(cfg["crossover"]["tol"]))
