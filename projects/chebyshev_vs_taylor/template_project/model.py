...
# ----------------------------
def _derivative_central(f: Callable[[float], float], c: float, order: int, h: float) -> float:
    """
    ...
    if order == 1:
        ...
        coeff = np.array([7/240, -3/10, 169/60, -122/15, 169/60, -3/10, 7/240])
...
def _derivative_fd_richardson(f: Callable[[float], float], c: float, k: int,
                              h0: float = 1e-2, levels: int = 3) -> float:
    """
...
def taylor_coeffs_fd(f_scalar: Callable[[float], float], c: float, n: int,
                     h0: float = 1e-2, richardson_levels: int = 3) -> np.ndarray:
    """
...
# Chebyshev fitting
# ----------------------------
def chebyshev_fit(f_vec: Callable[[np.ndarray], np.ndarray], a: float, b: float,
                  n: int, N_fit: int | None = None) -> Cheb:
    """
...
def compare_approximations(f_vec: Callable[[np.ndarray], np.ndarray],
                           cfg: ApproxConfig) -> Dict[str, Any]:
    """
    ...
    f_scalar = (lambda x: float(f_vec(np.array([x]))[0]))
    coeff_taylor = taylor_coeffs_fd(f_scalar, cfg.c, cfg.n, h0=cfg.h0, richardson_levels=cfg.richardson_levels)
    ...
    return {
        "x": x, "y": y,
        "p_cheb": p_cheb, "p_taylor": p_taylor,
        "err_cheb": err_cheb, "err_taylor": err_taylor,
...
def sweep_degree(f_vec: Callable[[np.ndarray], np.ndarray],
                 a: float, b: float, c: float,
                 n_list: List[int],
                 N_eval: int = 2001) -> Tuple[np.ndarray, np.ndarray]:
    """
