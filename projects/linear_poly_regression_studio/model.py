...
def poly_features(x: np.ndarray, degree: int, include_bias: bool = True) -> np.ndarray:
    ...
    if X.shape[1] != 1:
        raise ValueError(
            "This studio implements 1D polynomial features for clarity (shape (n,1) expected)."
        )
    n = X.shape[0]
...
def synthetic_data(
    n: int, x_range=(-1, 1), noise_sigma=0.2, seed: int = 0, func: str = "sin(2*pi*x)"
    ...
    def ftrue(xv):
        x = np.asarray(xv)
        return (
            np.sin(2 * np.pi * x) if func.strip() == "sin(2*pi*x)" else x**3 - 0.5 * x
        )
...
def bias_variance_curves(
    ...
    for deg in degrees:
        ...
        for t in range(n_trials):
            ...
            Phi_te = poly_features(
                x_grid.reshape(-1, 1), degree=deg, include_bias=include_bias
            )
            if scale:
