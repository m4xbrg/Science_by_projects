...
def to_array(value: Any, normalizer=None, fallback: List[str] | None = None) -> List[str]:
    if value is None:
...
def repair_one(meta_path: pathlib.Path, apply: bool, verbose: bool) -> bool:
    ...
    def norm_mc(x: Any) -> List[str]:
        if isinstance(x, str):
            ...
            more = []
            for p in parts:
                more.extend([q.strip().lower() for q in re.split(r"[-–—]", p) if q.strip()])
    ...
    if "created" not in data:
        data["created"] = datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds")
